...
def merge_dataframes(df_ec, df_registry):
   df_merge = pd.merge(df_registry, df_ec, on = ['RID','VISCODE'], how='outer', indicator=True) 
   return(df_merge)

# Filter records
def filter_records(df, options):
   df_filtered = df[ df_merge.VISCODE == options.viscode ]
   df_filtered = df_filtered[ df_filtered.SVDOSE == options.svdose ]
   df_filtered = df_filtered[ df_filtered.ECSDSTXT != options.ecsdstxt ]
   return df_filtered
...
if __name__ == "__main__":
   ...
   df_ec,df_registry = read_csv_files()
   draw_pie(df_ec, df_registry)
   df_merge = merge_dataframes(df_ec, df_registry)
   df_filtered = filter_records(df_merge, options)
